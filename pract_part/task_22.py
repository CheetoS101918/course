...
def create_custom_projects():
    ...
    project1 = Project("Проект 1")
    project1.Q_t = 232255.25         # Годовой объем товарной продукции (тыс.руб)
    project1.S_om = 4328.14           # Расходы основных материалов (тыс.руб)
    project1.S_pok = 90608.02         # Расходы покупных полуфабрикатов (тыс.руб)
    project1.S_vm = 1500           # Годовые расходы вспомогательных материалов (тыс.руб)
    project1.N_om = 20               # Норма запаса основных материалов (дн)
    project1.N_pok = 5               # Норма запаса покупных полуфабрикатов (дн)
    project1.N_vm = 9                 # Норма запаса вспомогательных материалов (дн)
    project1.OS_prz = 500.0           # Норматив по прочим производственным запасам (тыс.руб)
    project1.S = 369428.78            # Производственная себестоимость одного изделия (руб)
    project1.S_m = 9035.78 + 189160.79 # Начальные материальные расходы (руб)
    project1.T_c = 5                  # Длительность производственного цикла (дн)
    project1.N_gp = 1                 # Норма запаса готовой продукции (дн)
    project1.OS_rbp = 450           # Норматив на расходы будущих периодов (тыс.руб)
    project1.S_r = 176956.39          # Производственная себестоимость годового выпуска (тыс.руб)

    # Проект 2 - ВАШИ ДАННЫЕ
    project2 = Project("Проект 2")
    project2.Q_t = 245924.14          # Годовой объем товарной продукции (тыс.руб)
    project2.S_om = 4585.62           # Расходы основных материалов (тыс.руб)
    project2.S_pok = 96044.90         # Расходы покупных полуфабрикатов (тыс.руб)
    project2.S_vm = 1200.0             # Годовые расходы вспомогательных материалов (тыс.руб)
    project2.N_om = 15                # Норма запаса основных материалов (дн)
    project2.N_pok = 4               # Норма запаса покупных полуфабрикатов (дн)
    project2.N_vm = 9                 # Норма запаса вспомогательных материалов (дн)
    project2.OS_prz = 400.0           # Норматив по прочим производственным запасам (тыс.руб)
    project2.S = 365245.17            # Производственная себестоимость одного изделия (руб)
    project2.S_m = 8938.84 + 187222.03 # Начальные материальные расходы (руб)
    project2.T_c = 4                  # Длительность производственного цикла (дн)
    project2.N_gp = 1                 # Норма запаса готовой продукции (дн)
    project2.OS_rbp = 390           # Норматив на расходы будущих периодов (тыс.руб)
    project2.S_r = 187370.77          # Производственная себестоимость годового выпуска (тыс.руб)
