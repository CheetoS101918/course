...
class CostCalculator:
    ...
    def load_student_example_data(self):
        ...
        self.projects_data = {
            "project_1": {
                "name": "Проект 1",
                "annual_volume_base": 400,  # шт - Годовой объем производства (строка 1 таблицы 2.1)
                "annual_volume_corrected": math.ceil(400 * self.Ka),
            ...
            "project_2": {
                "name": "Проект 2",
                "annual_volume_base": 420,  # шт - Годовой объем производства (строка 1 таблицы 2.1)
                "annual_volume_corrected": math.ceil(420 * self.Ka),
        ...
        self.cost_calculation_data = {
            "project_1": {
                ...
                "steel_rolling_consumption": 0.425,  # т - расходы стального проката (пункт 1.1)
                "steel_rolling_waste": 0.085,  # т - отходы стального проката (пункт 1.1)
                "steel_pipes_consumption": 0.036,  # т - расходы стальных труб (пункт 1.2)
                "steel_pipes_waste": 0.0036,  # т - отходы стальных труб (пункт 1.2)
                "nonferrous_rolling": 2392,  # руб - прокат цветных металлов (пункт 1.3)
                "other_materials": 2576,  # руб - другие материалы (пункт 1.4)

                # ПОКУПНЫЕ ПОЛУФАБРИКАТЫ (раздел 2 таблицы 2.3)
                "castings_black_consumption": 2.4,  # т - расходы отливок черных металлов (пункт 2.1)
                "castings_black_waste": 0.48,  # т - отходы отливок черных металлов (пункт 2.1)
                "castings_color_consumption": 0.16,  # т - расходы отливок цветных металлов (пункт 2.2)
                "castings_color_waste": 0.032,  # т - отходы отливок цветных металлов (пункт 2.2)

                # КОМПЛЕКТУЮЩИЕ (раздел 3 таблицы 2.3)
                "purchased_components": 137610,  # руб - покупные комплектующие изделия (пункт 3)
                ...
                "fuel_energy_percent": 1.4,  # % - топливо и энергия на технологические потребности (пункт 8)

                # ТРУДОЕМКОСТЬ И ЗАРПЛАТА (разделы 9-10 таблицы 2.3)
                "labor_intensity": 566.44,  # н-час - суммарная трудоемкость изделия (пункт 9)
                "hourly_rate": 30.25,  # руб - часовая тарифная ставка (пункт 10)
            ...
            "project_2": {
                ...
                "steel_rolling_consumption": 0.44,  # т - скорректированный расход стального проката
                "steel_rolling_waste": 0.088,  # т - скорректированные отходы стального проката
                "steel_pipes_consumption": 0.034,  # т - скорректированный расход стальных труб
                "steel_pipes_waste": 0.0034,  # т - скорректированные отходы стальных труб
                "nonferrous_rolling": 2340,  # руб - скорректированный прокат цветных металлов
                "other_materials": 2520,  # руб - скорректированные другие материалы

                # ПОКУПНЫЕ ПОЛУФАБРИКАТЫ для Проекта 2
                "castings_black_consumption": 2.46,  # т - скорректированные расходы отливок черных металлов
                "castings_black_waste": 0.492,  # т - скорректированные отходы отливок черных металлов
                "castings_color_consumption": 0.164,  # т - скорректированные расходы отливок цветных металлов
                "castings_color_waste": 0.0328,  # т - скорректированные отходы отливок цветных металлов

                # КОМПЛЕКТУЮЩИЕ для Проекта 2
                "purchased_components": 136220,  # руб - скорректированные покупные комплектующие
                ...
                "fuel_energy_percent": 1.1,  # %

                # ТРУДОЕМКОСТЬ И ЗАРПЛАТА для Проекта 2
                "labor_intensity": 533.12,  # н-час - скорректированная трудоемкость
                "hourly_rate": 30.25,  # руб - часовая тарифная ставка (та же)
