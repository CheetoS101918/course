    ...
    def load_student_example_data(self):
        ...
        self.projects_data = {
            "project_1": {
                ...
                "annual_volume_corrected": math.ceil(450  * self.Ka),
            ...
            "project_2": {
                ...
                "annual_volume_corrected": math.ceil(410  * self.Ka),
        ...
        self.cost_calculation_data = {
            "project_1": {
                ...
                "steel_rolling_consumption": 0.405,  # т - расходы стального проката (пункт 1.1)
                "steel_rolling_waste": 0.06075,  # т - отходы стального проката (пункт 1.1)
                "steel_pipes_consumption": 0.0348,  # т - расходы стальных труб (пункт 1.2)
                "steel_pipes_waste": 0.002436,  # т - отходы стальных труб (пункт 1.2)
                ...
                "castings_black_consumption": 3.69,  # т - расходы отливок черных металлов (пункт 2.1)
                "castings_black_waste": 0.5535,  # т - отходы отливок черных металлов (пункт 2.1)
                "castings_color_consumption": 0.246,  # т - расходы отливок цветных металлов (пункт 2.2)
                "castings_color_waste": 0.0615,  # т - отходы отливок цветных металлов (пункт 2.2)

                # КОМПЛЕКТУЮЩИЕ (раздел 3 таблицы 2.3)
                "purchased_components": 132060,  # руб - покупные комплектующие изделия (пункт 3)
            ...
            "project_2": {
                ...
                "steel_rolling_consumption": 0.396,  # т - скорректированный расход стального проката
                "steel_rolling_waste": 0.0594,  # т - скорректированные отходы стального проката
                "steel_pipes_consumption": 0.0352,  # т - скорректированный расход стальных труб
                "steel_pipes_waste": 0.002464,  # т - скорректированные отходы стальных труб
                "nonferrous_rolling": 2639,  # руб - скорректированный прокат цветных металлов
                "other_materials": 1638,  # руб - скорректированные другие материалы

                # ПОКУПНЫЕ ПОЛУФАБРИКАТЫ для Проекта 2
                "castings_black_consumption": 3.825,  # т - скорректированные расходы отливок черных металлов
                "castings_black_waste": 0.57375,  # т - скорректированные отходы отливок черных металлов
                "castings_color_consumption": 0.255,  # т - скорректированные расходы отливок цветных металлов
                "castings_color_waste": 0.06375,  # т - скорректированные отходы отливок цветных металлов

                # КОМПЛЕКТУЮЩИЕ для Проекта 2
                "purchased_components": 129220,  # руб - скорректированные покупные комплектующие
                ...
                "fuel_energy_percent": 1,  # %

                # ТРУДОЕМКОСТЬ И ЗАРПЛАТА для Проекта 2
                "labor_intensity": 736,  # н-час - скорректированная трудоемкость
