...
class CostCalculator:
    ...
    def load_student_example_data(self):
        ...
        self.Ka = 1.03  # коэффициент для объема производства
        ...
        self.projects_data = {
            "project_1": {
                "name": "Проект 1",
                "annual_volume_base": 465,  # шт - Годовой объем производства (строка 1 таблицы 2.1)
                "annual_volume_corrected": math.ceil(465 * self.Ka),
            ...
            "project_2": {
                "name": "Проект 2",
                "annual_volume_base": 498,  # шт - Годовой объем производства (строка 1 таблицы 2.1)
                "annual_volume_corrected": math.ceil(498 * self.Ka),
        ...
        self.cost_calculation_data = {
            "project_1": {
                ...
                "steel_rolling_consumption": 0.3382,  # т - расходы стального проката (пункт 1.1)
                "steel_rolling_waste": 0.0608,  # т - отходы стального проката (пункт 1.1)
                "steel_pipes_consumption": 0.0364,  # т - расходы стальных труб (пункт 1.2)
                "steel_pipes_waste": 0.00364,  # т - отходы стальных труб (пункт 1.2)
                "nonferrous_rolling": 3069,  # руб - прокат цветных металлов (пункт 1.3)
                "other_materials": 693,  # руб - другие материалы (пункт 1.4)

                # ПОКУПНЫЕ ПОЛУФАБРИКАТЫ (раздел 2 таблицы 2.3)
                "castings_black_consumption": 1.827,  # т - расходы отливок черных металлов (пункт 2.1)
                "castings_black_waste": 0.365,  # т - отходы отливок черных металлов (пункт 2.1)
                "castings_color_consumption": 0.113,  # т - расходы отливок цветных металлов (пункт 2.2)
                "castings_color_waste": 0.019,  # т - отходы отливок цветных металлов (пункт 2.2)

                # КОМПЛЕКТУЮЩИЕ (раздел 3 таблицы 2.3)
                "purchased_components": 167112,  # руб - покупные комплектующие изделия (пункт 3)
                ...
                "fuel_energy_percent": 1.8,  # % - топливо и энергия на технологические потребности (пункт 8)

                # ТРУДОЕМКОСТЬ И ЗАРПЛАТА (разделы 9-10 таблицы 2.3)
                "labor_intensity": 884.9,  # н-час - суммарная трудоемкость изделия (пункт 9)
                "hourly_rate": 35.6,  # руб - часовая тарифная ставка (пункт 10)

                # ПРОЦЕНТЫ ДЛЯ РАСЧЕТОВ (разделы 11-17 таблицы 2.3)
                "additional_salary_percent": 41,  # % - дополнительная зарплата (пункт 11)
                "social_insurance_percent": 22,  # % - отчисление на социальное страхование (пункт 12)
                "equipment_maintenance_percent": 110,  # % - расходы на содержание и эксплуатацию оборудования (пункт 13)
                "overhead_production_percent": 110,  # % - общепроизводственные расходы (пункт 14)
                "general_business_percent": 140,  # % - общехозяйственные расходы (пункт 15)
                "non_production_percent": 5,  # % - внепроизводственные расходы (пункт 16)
                "profitability_percent": 25  # % - норматив рентабельности к себестоимости (пункт 17)
            ...
            "project_2": {
                ...
                "steel_rolling_consumption": 0.3344,  # т - скорректированный расход стального проката
                "steel_rolling_waste": 0.0601,  # т - скорректированные отходы стального проката
                "steel_pipes_consumption": 0.036,  # т - скорректированный расход стальных труб
                "steel_pipes_waste": 0.0036,  # т - скорректированные отходы стальных труб
                "nonferrous_rolling": 3038,  # руб - скорректированный прокат цветных металлов
                "other_materials": 686,  # руб - скорректированные другие материалы

                # ПОКУПНЫЕ ПОЛУФАБРИКАТЫ для Проекта 2
                "castings_black_consumption": 1.806,  # т - скорректированные расходы отливок черных металлов
                "castings_black_waste": 0.361,  # т - скорректированные отходы отливок черных металлов
                "castings_color_consumption": 0.112,  # т - скорректированные расходы отливок цветных металлов
                "castings_color_waste": 0.019,  # т - скорректированные отходы отливок цветных металлов

                # КОМПЛЕКТУЮЩИЕ для Проекта 2
                "purchased_components": 165424,  # руб - скорректированные покупные комплектующие
                ...
                "labor_intensity": 875.9,  # н-час - скорректированная трудоемкость
