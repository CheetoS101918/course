...
def create_custom_projects():
    ...
    project1 = Project("Проект 1")
    project1.Q_t = 136533.4108        # Годовой объем товарной продукции (тыс.руб)
    project1.S_om = 4063.37           # Расходы основных материалов (тыс.руб)
    project1.S_pok = 53086.63         # Расходы покупных полуфабрикатов (тыс.руб)
    project1.S_vm = 1600.0            # Годовые расходы вспомогательных материалов (тыс.руб)
    ...
    project1.S = 315851.87            # Производственная себестоимость одного изделия (руб)
    project1.S_m = 166133.75          # Начальные материальные расходы (руб)
    project1.T_c = 8                  # Длительность производственного цикла (дн)
    project1.N_gp = 2                 # Норма запаса готовой продукции (дн)
    project1.OS_rbp = 320.0           # Норматив на расходы будущих периодов (тыс.руб)
    project1.S_r = 108653.04          # Производственная себестоимость годового выпуска (тыс.руб)

    # Проект 2 - ВАШИ ДАННЫЕ
    project2 = Project("Проект 2")
    project2.Q_t = 147655.32          # Годовой объем товарной продукции (тыс.руб)
    project2.S_om = 4344.73           # Расходы основных материалов (тыс.руб)
    project2.S_pok = 57424.49         # Расходы покупных полуфабрикатов (тыс.руб)
    ...
    project2.S = 322812.72            # Производственная себестоимость одного изделия (руб)
    project2.S_m = 169695.64          # Начальные материальные расходы (руб)
    project2.T_c = 7                  # Длительность производственного цикла (дн)
    project2.N_gp = 2                 # Норма запаса готовой продукции (дн)
    project2.OS_rbp = 800.0           # Норматив на расходы будущих периодов (тыс.руб)
    project2.S_r = 117503.83          # Производственная себестоимость годового выпуска (тыс.руб)
