...
def load_variables(file_path):
    try:
        ...
        # Устанавливаем 'Изделие' как индекс для быстрого поиска (по А и Б)
        df.set_index('Изделие', inplace=True)
        ...
        # ВНИМАНИЕ: Этих данных нет в Таблице Н.1, их нужно брать из другого расчета
        # C_A = None  # Производственная себестоимость (нужен внешний источник)
        # C_m_A = None  # Начальные материальные расходы (нужен внешний источник)
        ...
        # ВНИМАНИЕ: Этих данных нет в Таблице Н.1
        # C_B = None
        # C_m_B = None
        ...
        return {
            'A': (C_vm_A, N_om_A, N_pok_A, N_vm_A, OS_prz_A, T_c_A, N_gp_A, OS_rbp_A),
            'B': (C_vm_B, N_om_B, N_pok_B, N_vm_B, OS_prz_B, T_c_B, N_gp_B, OS_rbp_B)
        }
...
# --- ПРИМЕР ЗАПУСКА ---
file_path = 'dop_N.csv'
data = load_variables(file_path)

if isinstance(data, dict):
    # Распаковка для А
    (C_vm_A, N_om_A, N_pok_A, N_vm_A, OS_prz_A, T_c_A, N_gp_A, OS_rbp_A) = data['A']

    # Распаковка для Б
    (C_vm_B, N_om_B, N_pok_B, N_vm_B, OS_prz_B, T_c_B, N_gp_B, OS_rbp_B) = data['B']
