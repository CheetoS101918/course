...
with open('sebestoimost_structure.json', 'r', encoding='utf-8') as f:
    data = json.load(f)
...
# Изделие А
Q_A = 45476.61  # тыс.руб (Годовой объем товарной продукции)
...
C_vm_A = 1800  # тыс. руб (Годовые расходы вспомогательных материалов)
N_om_A = 30  # дн (Норма запаса основных материалов)
N_pok_A = 20  # дн (Норма запаса покупных полуфабрикатов и комплектующих)
N_vm_A = 10  # дн (Норма запаса вспомогательных материалов)
...
T_c_A = 10  # Дн (Длительность производственного цикла)
N_gp_A = 3  # Дн (Норма запаса готовой продукции)
OS_rbp_A = 700  # тыс.руб (Норматив оборотных средств на расходы будущих периодов)

# Изделие Б
Q_B = 16496.17  # тыс.руб (Годовой объем товарной продукции)
...
N_om_B = 25  # дн (Норма запаса основных материалов)
N_pok_B = 10  # дн (Норма запаса покупных полуфабрикатов и комплектующих)
N_vm_B = 12  # дн (Норма запаса вспомогательных материалов)
OS_prz_B = 700  # тыс.руб (Норматив оборотных средств по прочим производственным запасам)
C_B = 193053.24  # Руб (Производственная себестоимость одного изделия)
C_m_B = 162568.59  # Руб (Начальные материальные расходы)
T_c_B = 5  # Дн (Длительность производственного цикла)
N_gp_B = 2  # Дн (Норма запаса готовой продукции)
OS_rbp_B = 690  # тыс.руб (Норматив оборотных средств на расходы будущих периодов)

# --- Вывод расчётов в консоль (точно как в task3.docx) ---
print("Изделие А")
