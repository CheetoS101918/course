...
def main():
    ...
    # Группа вводимых фондов (из задания 3-го варианта)
    gruppa_vvod = 'Сооружения' # Меняется в зависимости от варианта
    # Группа выводимых фондов (из задания 3-го варианта)
    gruppa_vyvod = 'Здания' # Меняется в зависимости от варианта
    # Размер вводимых фондов (%) (из задания 3-го варианта)
    procent_vvoda = 2.6
    # Размер выводимых фондов (%) (из задания 3-го варианта)
    procent_vyvoda = 3
    # Месяц ввода (из задания 3-го варианта)
    mes_vvoda = 3
    # Месяц вывода (из задания 3-го варианта)
    mes_vyvoda = 6
    ...
    # 'Здания' -> '1.Здания', 'Сооружения' -> '2.Сооружения', 'Силовые машины и оборудование' -> '4.1. ...', 'Рабочие машины и оборудование' -> '4.2. ...', 'Транспортные средства' -> '5.Транспортные средства', 'Другие основные фонды' -> '6. Другие основные фонды'
    ...
    elif gruppa_vvod in ["Силовые машины и оборудование", "Рабочие машины и оборудование", "Измерительные приборы и устройства", "Вычислительная техника", "Другие машины и оборудование"]:
        # Для подпунктов машин и оборудования
        if gruppa_vvod == "Силовые машины и оборудование":
            key_vvod = "4.1. Силовые машины и оборудование"
    ...
    key_vyvod = None
    if gruppa_vyvod == "Силовые машины и оборудование":
        key_vyvod = "4.1. Силовые машины и оборудование"
    ...
    for i, name in enumerate(["Силовые машины и оборудование", "Рабочие машины и оборудование", "Измерительные приборы и устройства", "Вычислительная техника", "Другие машины и оборудование"], 1):
        key = f"4.{i}. {name}"
    ...
    if not key_vyvod:
        ...
        if key_vyvod:
            ...
            if key_vyvod in rows_data:
                 rows_data[key_vyvod]["Выведено из строя, тыс.руб"] = stoimost_vyvodimyh
            else:
                 # Если запись не существует (например, если она была вводимой), создаём новую
                 rows_data[key_vyvod] = {
                     "Стоимость на начало года, тыс.руб": stoimost_na_nachalo[gruppa_vyvod],
                     "Введено в строй, тыс.руб": 0.0, # Если группа одновременно и вводима и выводима - вводимое значение уже стоит
                     "Выведено из строя, тыс.руб": stoimost_vyvodimyh
                 }
        else:
    ...
    machines_nachalo = sum(
        rows_data[f"4.{i}. {name}"]["Стоимость на начало года, тыс.руб"] for i, name in enumerate(
            ["Силовые машины и оборудование", "Рабочие машины и оборудование", "Измерительные приборы и устройства",
